...
#
# OBS:
# - No Streamlit Cloud rode SOMENTE a UI (sem --bot).
# =============================================================
...
DB_PATH               = os.path.join(BASE_DIR, "mvp_funds.db")

DEFAULT_ADMIN_USER    = "admin"
DEFAULT_ADMIN_PASS    = "87347748"  # <- sua senha (mude se quiser)
...
def init_db():
    ...
    # Migrações seguras (se já existir, ignora)
    for migration in [
...
def bot_step(user_id: int):
    ...
    if in_pos == 0:
        if usdt < MIN_USDT_ORDER:
            ...
            upsert_bot_state(user_id, 1, usdt, 0.0, 0, None, None, None, _now(), last_error=s.get("last_error"))
            return
        try:
...
# Cookie manager seguro (não pode quebrar o app)
cookie_manager = None
if HAS_COOKIE_LIB:
    try:
...
def safe_cookie_delete(name: str):
    if cookie_manager is None:
        return
    try:
        cookie_manager.delete(name, key=f"cookie_del_{int(time.time())}")
    except KeyError:
        # quando não existe cookie
        pass
    except Exception:
        pass
...
def do_login(user):
    ...
    if HAS_COOKIE_LIB and cookie_manager is not None:
        try:
            ...
            cookie_manager.set(COOKIE_NAME, token, key=f"cookie_set_{int(time.time())}")
        except Exception:
            pass

def do_logout():
    st.session_state.user = None
    safe_cookie_delete(COOKIE_NAME)
    st.rerun()
