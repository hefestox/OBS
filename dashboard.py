...
# ccxt (opcional; não confiamos nele no Cloud)
try:
    import ccxt
    HAS_CCXT = True
except Exception:
...
MIN_USDT_ORDER = 10.0
BOT_MIN_INTERVAL_SEC = 10
MIN_HOLD_SECONDS_FOR_TP = 0


# =========================
# SIMULAÇÃO CONTROLADA (para testar fluxo sem cair)
# =========================
# Ligue só enquanto estiver testando infra/servidor:
SIMULATION_MODE = True

# Se SIMULATION_MODE=True, o preço simulado deriva do preço real ancorado
# e tende a bater TP com frequência (testa BUY/SELL/DB/metrics).
SIM_VOL_PCT_PER_STEP = 0.0004   # ruído pequeno
SIM_MEAN_REVERT = 0.04          # puxa pro anchor
SIM_BIAS_UP = 0.00012           # viés para cima (lucro mais frequente)
...
def fetch_price_http(symbol: str) -> float | None:
    """Preço real via HTTP (sem ccxt)."""
    try:
        sym = symbol.replace("/", "").upper()  # BTC/USDT -> BTCUSDT
        r = requests.get("https://api.binance.com/api/v3/ticker/price", params={"symbol": sym}, timeout=10)
...
def get_price_simulated(symbol: str) -> float | None:
    """
    Preço simulado ancorado no preço real (pego 1x).
    Serve para testar pipeline sem depender do ccxt.
    """
    import random

    key_anchor = f"sim_anchor_{symbol}"
    key_last = f"sim_last_{symbol}"

    if key_anchor not in st.session_state:
        anchor = fetch_price_http(symbol)
        if anchor is None:
            return None
        st.session_state[key_anchor] = float(anchor)
        st.session_state[key_last] = float(anchor)

    anchor = float(st.session_state[key_anchor])
    last = float(st.session_state.get(key_last, anchor))

    # drift para anchor + viés leve de alta + ruído
    drift = (anchor - last) / anchor * SIM_MEAN_REVERT
    shock = random.gauss(0, SIM_VOL_PCT_PER_STEP)
    pct = drift + shock + SIM_BIAS_UP

    newp = last * (1 + pct)
    newp = max(newp, anchor * 0.6)
    newp = min(newp, anchor * 1.6)

    st.session_state[key_last] = float(newp)
    return float(newp)
...
def bot_step_agressivo(user_id: int) -> str:
    ...
    PAPER:
    - FLAT -> compra
    - COMPRADO -> vende em TP/SL
    - preço: HTTP (real) ou simulado (se SIMULATION_MODE=True)
    """
    s = get_bot_state(user_id)
    if not s:
    ...
    # preço
    price = get_price_simulated(BOT_SYMBOL) if SIMULATION_MODE else fetch_price_http(BOT_SYMBOL)
    if price is None:
        upsert_bot_state(user_id, enabled, usdt, asset, in_pos, entry_price, entry_qty, entry_time,
                         now.isoformat(sep=" ", timespec="seconds"))
        return "Sem preço agora (HTTP indisponível)."
    ...
    if in_pos == 1 and entry_price is not None and entry_qty is not None and float(entry_qty) > 0:
        ...
        tp_price = entry_price_f * (1 + TAKE_PROFIT)
        sl_price = entry_price_f * (1 - STOP_LOSS)
        ...
        if exit_reason:
            ...
            entry_cost = entry_price_f * entry_qty_f
            pnl = net - entry_cost
...
if SIMULATION_MODE:
    st.caption("🧪 SIMULAÇÃO ATIVA: preço ancorado no BTC real com variação simulada (teste de fluxo).")
...
with tabs[0]:
    ...
    # preço atual (real ou simulado)
    price_now = get_price_simulated(BOT_SYMBOL) if SIMULATION_MODE else fetch_price_http(BOT_SYMBOL)
    ...
    if in_pos == 1 and entry_price:
        ...
        tp_price = entry_price_f * (1 + TAKE_PROFIT)
        sl_price = entry_price_f * (1 - STOP_LOSS)
        b.metric("Entrada", f"{entry_price_f:.2f}")
        c.metric("TP", f"{tp_price:.2f}")
        d.metric("SL", f"{sl_price:.2f}")
        if price_now is not None:
            pnl_pct = (price_now / entry_price_f - 1) * 100.0
