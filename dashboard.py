...
DEFAULT_ADMIN_PASS    = "TrocaEssaSenha@2024"     # ← MUDE ISSO
...
# Chave secreta para assinar token de sessão — MUDE ISSO
...
def init_db():
    ...
    # Migração segura — adiciona colunas novas sem quebrar banco existente
    for migration in [
...
# ── Sessões persistentes (salvas no banco) ────────────────────
def create_session(user_id: int) -> str:
    """Cria token de sessão salvo no banco. Válido por 30 dias."""
...
def get_session_user(token: str):
    """Valida token e retorna user tuple se válido."""
...
def list_deposits(user_id=None):
    ...
    if user_id is None:
        ...
        cur.execute("""SELECT d.id, d.amount_usdt, d.txid, d.status,
                              d.created_at, d.reviewed_at, d.note
...
# =============================================================
# INTERFACE STREAMLIT — sem dependência externa de cookie
# =============================================================
...
# =============================================================
# SESSÃO PERSISTENTE — via banco de dados + st.query_params
...
# Tenta recuperar sessão do query param (URL)
if st.session_state.user is None:
    ...
    if sid:
        ...
        if recovered:
            ...
            st.query_params.clear()  # token inválido/expirado
...
st.caption(f"{BOT_SYMBOL} | TP {TAKE_PROFIT*100:.1f}% | SL {STOP_LOSS*100:.1f}% | bot: python dashboard.py --bot")
