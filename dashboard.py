#!/usr/bin/env python3
# =============================================================
# OBS PRO BOT — ARQUIVO ÚNICO COMPLETO
#
# INSTALAÇÃO:
#   pip install streamlit pandas requests ccxt streamlit-autorefresh
#
# COMO RODAR (2 terminais no servidor):
...
DEPOSIT_ADDRESS_FIXED = "0xSeuEnderecoERC20Aqui"  # ← MUDE ISSO
...
STOP_LOSS             = 0.003   # -0.3%
FEE_RATE_EST          = 0.001   # 0.1% fee Binance
...
def init_db():
    ...
    if cur.fetchone() is None:
        cur.execute("""
            INSERT INTO users (username, pass_hash, role, created_at, my_code)
            VALUES (?,?,?,?,?)
...
def save_user_keys(user_id: int, api_key: str, api_secret: str, testnet: bool = False):
    ...
    if exists:
        cur.execute("""UPDATE user_keys SET api_key=?, api_secret=?, testnet=?, updated_at=?
                       WHERE user_id=?""",
                    (api_key.strip(), api_secret.strip(), int(testnet), _now(), user_id))
    else:
        cur.execute("""INSERT INTO user_keys (user_id, exchange, api_key, api_secret, testnet, updated_at)
                       VALUES (?,?,?,?,?,?)""",
...
def add_ledger(user_id: int, kind: str, amount_usdt: float, ref_table=None, ref_id=None):
    conn = db(); cur = conn.cursor()
    cur.execute("""INSERT INTO ledger (user_id, kind, amount_usdt, ref_table, ref_id, created_at)
                   VALUES (?,?,?,?,?,?)""",
...
def create_deposit(user_id: int, amount_usdt: float, txid: str):
    ...
    conn = db(); cur = conn.cursor()
    cur.execute("""INSERT INTO deposits (user_id, amount_usdt, txid, deposit_address, status, created_at)
                   VALUES (?,?,?,?,?,?)""",
...
def admin_review_withdrawal(wid: int, approve: bool, admin_id: int, note: str = ""):
    ...
    conn.commit(); conn.close()
    if approve:
        add_ledger(user_id, "WITHDRAWAL", -float(amt), "withdrawals", wid)
...
def admin_mark_withdraw_paid(wid: int, admin_id: int, paid_txid: str, note: str = ""):
    ...
    cur.execute("""UPDATE withdrawals SET status='PAID', paid_txid=?, reviewed_at=?, reviewed_by=?, note=?
                   WHERE id=?""", (paid_txid.strip(), _now(), admin_id, note, wid))
    conn.commit(); conn.close()
...
def bot_step(user_id: int):
    try:
        ...
        logging.error("ccxt não instalado! pip install ccxt"); return
    ...
    if in_pos == 0:
        if usdt < MIN_USDT_ORDER:
            ...
            upsert_bot_state(user_id, 1, usdt, 0.0, 0, None, None, None, _now()); return
        try:
    ...
    if in_pos == 1 and entry_price and entry_qty:
        ...
        exit_reason = None
        if held_ok and price >= tp: exit_reason = "TAKE_PROFIT"
        ...
        if exit_reason:
            ...
            logging.debug(f"[user {user_id}] Holding {pct:+.3f}% | TP={tp:.2f} SL={sl:.2f}")
...
def run_bot_loop():
    ...
    log.info("=" * 50)
    log.info(f"  OBS PRO BOT — Iniciado")
    ...
    while True:
        try:
            ...
            if ativos:
                ...
                for uid in ativos:
                    try: bot_step(uid)
                    except Exception as e: log.error(f"[user {uid}] {e}", exc_info=True)
            else:
...
# ─── Se rodado com --bot, sobe o runner e para aqui ──────────
if BOT_MODE:
    run_bot_loop()
    sys.exit(0)
...
st.caption(f"{BOT_SYMBOL} | TP {TAKE_PROFIT*100:.1f}% | SL {STOP_LOSS*100:.1f}% | bot: python app.py --bot")

if not st.session_state.user:
    st.info("Faça login na barra lateral."); st.stop()
...
with tabs[0]:
    ...
    if s and int(s.get("enabled") or 0) != int(new_on):
        ...
        upsert_bot_state(user_id, 1, 0.0, 0.0, 0, None, None, None, None); st.rerun()

    if not s:
        st.info("Ative o bot para iniciar."); st.stop()

    s = get_bot_state(user_id)

    err = s.get("last_error")
    if err: st.error(f"🚨 Último erro: {err}")

    lts = s.get("last_step_ts")
    if lts: st.caption(f"⏱ Último ciclo do runner: `{lts}`")
    ...
    if df_tr.empty:
        st.info("Sem operações. Rode: python app.py --bot")
    else:
...
with tabs[2]:
    ...
    if ex: st.success(f"✅ Chaves cadastradas | Key: `{ex[0][:8]}...` | Testnet: {'Sim' if ex[2] else 'Não'}")
...
4. Restrinja ao IP do seu servidor para máxima segurança
    """)
...
with tabs[3]:
    ...
    st.caption("Envie USDT para o endereço acima e cole o TXID abaixo. O admin confirmará o crédito.")
...
with tabs[4]:
    ...
    if st.button("📤 Solicitar saque"):
        try:
            ...
            else: create_withdrawal(user_id, amt_w, net_w, adr_w); st.success("Solicitado! Aguarde aprovação.")
        except Exception as e: st.error(str(e))
