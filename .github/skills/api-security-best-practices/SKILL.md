...
description: "Implement secure API-specific design patterns including authentication, authorization, schema validation, rate limiting, token handling, and protection against common API vulnerabilities. Prefer security-best-practices for browser-facing CSP, cookies, and generic web header hardening."
metadata:
  risk: unknown
  source: community
...
\`\`\`javascript
// ❌ Bad
const JWT_SECRET = '<hardcoded-secret-from-source-control>';
