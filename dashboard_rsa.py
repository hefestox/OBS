#!/usr/bin/env python3
# =============================================================
# OBS PRO BOT — VERSÃO 5.1.0 — FIX TRAILING PEAK
#
# CORREÇÕES v5.0.0:
...
#
# CORREÇÕES v5.1.0:
#   ✅ trailing_peak salvo em coluna dedicada (não mais em last_error)
#   ✅ last_error preservado corretamente durante trailing stop
#   ✅ trailing_peak resetado ao fechar posição
# =============================================================
...
TRADE_FEE = 0.010                # taxa por operação cobrada na VENDA
...
# ── Agente IA ─────────────────────────────────────────────────
ANTHROPIC_API_KEY = "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"  # ← coloque sua chave aqui
...
def init_db():
    with _DB_LOCK:
        ...
        for m in [
            ...
            # v5.1.0: coluna dedicada para trailing peak
            ...
            try:
                ...
                conn.execute("""INSERT OR IGNORE INTO bot_state_new
                    (user_id, symbol, enabled, usdt, asset, in_position, entry_price, entry_qty,
                     entry_time, last_step_ts, last_error, last_sl_time, daily_losses, daily_loss_date,
                     trailing_peak, updated_at)
                    SELECT user_id, 'BTC/USDT', enabled, usdt, asset, in_position, entry_price, entry_qty,
                           ...
                           COALESCE(daily_losses,0), daily_loss_date,
                           NULL,
                           updated_at
                    FROM bot_state""")
                ...
                conn.commit()
                logging.info("✅ Migração v5.1.0: bot_state com trailing_peak!")
    ...
    with _DB_LOCK:
        ...
        if cur.fetchone():
            ...
            cur.execute(
                ...
                (user_id, "binance", api_key, api_secret, int(testnet), _now()))
        conn.commit(); conn.close()
    ...
    for sym in ALL_SYMBOLS:
        ...
        if s:
            upsert_bot_state(user_id, 1,
                             ...
                             symbol=sym, trailing_peak=s.get("trailing_peak"))
        else:
...
def upsert_bot_state(user_id, enabled, usdt, asset, in_position,
                     ...
                     symbol="BTC/USDT",
                     trailing_peak=None):   # v5.1.0: campo dedicado para trailing peak
    with _DB_LOCK:
        conn = db(); cur = conn.cursor()
...
def _save_error(user_id, msg, s=None, symbol="BTC/USDT"):
    ...
    upsert_bot_state(user_id, int(s.get("enabled") or 1),
                     ...
                     symbol=symbol,
                     trailing_peak=s.get("trailing_peak"))  # v5.1.0: preserva trailing_peak ao salvar erro
...
def _make_exchange(api_key, api_secret, testnet):
    ...
    api_key = (api_key or "").strip()
    api_secret = _normalize_api_credential(api_secret)
    mode = _credential_mode(api_secret)
    ...
    try:
        logging.info(f"Conexão Binance configurada com credencial {_credential_label(api_secret)}")
    except Exception:
        pass
...
def bot_step(user_id, symbol="BTC/USDT", exchange=None):
    ...
    cred_mode = _credential_label(api_secret)
    if exchange is None:
        try:
            exchange = _make_exchange(api_key, api_secret, bool(testnet))
        except Exception as e:
            _save_error(user_id, f"Falha exchange ({cred_mode}): {e}"); return
    ...
    if in_pos == 0:
        ...
        try:
            ...
            upsert_bot_state(user_id, 1, un, an, 1, fp, fq,
                             ...
                             daily_losses=daily_losses, daily_loss_date=daily_loss_date,
                             symbol=symbol,
                             trailing_peak=None)  # v5.1.0: inicia sem peak ao comprar
    ...
    if in_pos == 1 and entry_price and entry_qty:
        ...
        if USE_TRAILING_STOP:
            # v5.1.0: lê o peak do campo dedicado — não mais do last_error
            ...
            # v5.1.0: salva o peak no campo correto, preserva last_error real
            upsert_bot_state(user_id, 1, usdt, asset, 1, entry_price, entry_qty,
                             entry_time, _now(),
                             last_error=s.get("last_error"),   # preserva erro real
                             last_sl_time=last_sl_time,
                             daily_losses=daily_losses, daily_loss_date=daily_loss_date,
                             symbol=symbol,
                             trailing_peak=new_peak)           # salva no campo dedicado
        ...
        if exit_reason:
            try:
                ...
                # v5.1.0: trailing_peak resetado ao fechar posição
                upsert_bot_state(user_id, 1, un, an, 0, None, None, None, _now(),
                                 ...
                                 symbol=symbol,
                                 trailing_peak=None)  # reseta peak ao sair da posição
            ...
            upsert_bot_state(user_id, 1, usdt, asset, 1, entry_price, entry_qty,
                             ...
                             daily_losses=daily_losses, daily_loss_date=daily_loss_date,
                             symbol=symbol,
                             trailing_peak=s.get("trailing_peak"))  # v5.1.0: preserva peak
...
def run_bot_loop():
    ...
    log = logging.getLogger(__name__)
    log.info("=" * 60)
    log.info("  OBS PRO BOT v5.1.0 — TRAILING PEAK FIX")
    ...
    log.info(f"  ✅ FIX: trailing_peak em coluna dedicada — last_error preservado")
    log.info("=" * 60)
...
init_db()
st.set_page_config(page_title="OBS PRO — BOT v5.1.0", layout="wide")
...
with tabs[2]:
    ...
    st.info("Você pode usar **HMAC** (API Key + Secret) ou **RSA** (API Key + Chave Privada PEM).")
    ex = get_user_keys(user_id)
    if ex:
        st.success(f"✅ Chaves cadastradas | Tipo: `{_credential_label(ex[1])}` | Key: `{ex[0][:8]}...`")
    with st.form("form_keys"):
        nk = st.text_input("API Key", type="password")
        key_mode = st.radio("Tipo de credencial", ["HMAC (Secret)", "RSA (Private Key PEM)"], horizontal=True)
        if key_mode == "HMAC (Secret)":
            ns = st.text_input("API Secret", type="password")
        else:
            ns = st.text_area(
                "Chave Privada RSA (PEM)",
                height=220,
                placeholder="Cole aqui a chave privada completa, incluindo BEGIN/END PRIVATE KEY",
            )
        tn = st.checkbox("Usar Testnet", value=False)
        if st.form_submit_button("💾 Salvar chaves"):
            try:
                save_user_keys(user_id, nk, ns, tn)
                st.success(f"✅ Chaves salvas com sucesso! Tipo detectado: {_credential_label(ns)}")
                st.rerun()
            except Exception as e:
                st.error(str(e))
...
with tabs[6]:
    ...
    try:
        ...
        def _agent_calc_ema(closes, period):
            if len(closes) < period: return None
            k = 2.0 / (period + 1)
            ema = sum(closes[:period]) / period
        ...
        def _agent_get_market():
            resultado = {}
            for sym in BOT_SYMBOLS:
                price = _agent_get_price(sym.replace("/",""))
                try:
                    r = requests.get("https://api.binance.com/api/v3/klines",
                                     params={"symbol": sym.replace("/",""), "interval": "1h", "limit": 210}, timeout=10)
                    closes_h1 = [float(c[4]) for c in r.json()]
                except: closes_h1 = []
                ema200 = _agent_calc_ema(closes_h1, 200)
                bloqueios = []
                if price and ema200 and price <= ema200:
                    bloqueios.append(f"Abaixo EMA200 H1 (falta ${ema200-price:.2f})")
                resultado[sym] = {
                    "preco": price,
                    "ema200_h1": round(ema200, 2) if ema200 else None,
                    "sinal": not bloqueios,
                    "bloqueios": bloqueios,
                }
            return resultado

        def _agent_get_bot_state():
            estados = get_all_bot_states(user_id)
            trades  = load_bot_trades(user_id, 5)
            return {"estados": estados, "trades": trades.to_dict("records") if not trades.empty else []}

        def _agent_system(mercado, bot):
            import json
            return f"""Você é um assistente especialista integrado ao OBS PRO BOT v5.1.0.
...
ESTADO DO BOT:
{json.dumps(bot, ensure_ascii=False, indent=2, default=str)}

ESTRATÉGIA:
- Entrada: preço > EMA200 H1 + EMA50 4H + MACD > 0 + RSI 47-62
- TP: 1% | SL: 0.5% | Trailing Stop em +0.7%
- Capital: $30 por par (BTC/USDT e ETH/USDT)
- Cooldown: 20 min após stop loss

Responda em português, de forma clara e direta. Máximo 6 linhas."""

        if "agent_messages" not in st.session_state:
            st.session_state.agent_messages = []
        if "agent_mercado" not in st.session_state:
            st.session_state.agent_mercado = None
        ...
        for i, (par, d) in enumerate(mercado_atual.items()):
            ...
            status = "✅ SINAL" if d["sinal"] else "⏸ Aguardando"
            falta = ""
            if d["ema200_h1"] and d["preco"] and d["ema200_h1"] > d["preco"]:
                falta = f" | Falta ${d['ema200_h1']-d['preco']:.2f}"
            col.metric(par, f"${d['preco']:,.2f}" if d["preco"] else "—", f"{status}{falta}")
            ...
            with st.chat_message("assistant"):
                with st.spinner("Analisando..."):
                    try:
                        ...
                        bot_state = _agent_get_bot_state()
                        ...
                        resp = client.messages.create(
                            ...
                            max_tokens=1000,
                            system=_agent_system(mercado_atual, bot_state),
                            messages=hist
                        )
        ...
        st.warning("⚠️ Instale a biblioteca: `pip install anthropic`")
    except Exception as e:
